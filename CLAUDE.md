...
- **Image pipeline** — drop rasters into `/public`, run `npm run optimize-images` (`scripts/convert-to-webp.js`, quality 78, skips fresh files). Originals are kept but the runtime always requests `.webp`.
  ...
  - **`width`/`height` must describe the file actually SERVED, not the master.** `optimize-images` **trims transparent edges**, so `logo_final.webp` is 700×487 (1.437) while its own `-400`/`-800` variants are 1.50. Declaring the master's numbers over a variant is a real aspect-ratio mismatch and costs a Best Practices point. Measure the variant.
