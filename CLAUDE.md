...
- **One-time importers (flag-gated)** — besides `seedMenuIfEmpty()`, `accounting.js` exports `ensurePerPieceMenu()`, `ensureCakePopPrices()` and `importExcelDataIfNeeded()`; each no-ops after setting a `localStorage` flag (`cc_acc_perpiece_v1`, `cc_acc_cakepop_v2`, `cc_acc_excel_v7`). The Excel importer **dynamically** imports `src/data/excelImport.js` (the owner's Apr–Jul 2026 history, ~4.7k lines auto-generated from `cake_and_crumb_excel.xlsx`) so it emits its own async chunk instead of bloating the bundle. It writes **deterministic ids** (`xl-o-N` / `xl-e-N`) in 400-doc `writeBatch` chunks and deletes prior `xl-`-prefixed rows first, so a re-run overwrites rather than duplicates and a shortened sheet leaves no orphans; manual entries are never touched. **To re-import after regenerating the data, bump `EXCEL_VERSION`** (`'v7'` → `'v8'`) — editing the data alone changes nothing. The two menu importers stay `localStorage`-gated (they only ever *add* missing rows, so a repeat is harmless), but the Excel importer is **gated on the cloud marker `acc_settings/main.excelImport`**, not on the local flag: it deletes and `set`s (no merge) every `xl-` doc, so it must run once **per database**, not once per device. It was per-device before, and that was the bug — a second PC opening `/admin/accounting` for the first time silently reverted every cloud edit made to an imported Apr–Jul row (a corrected amount, the `withdrawn` "✓ Taken to cash" flag) back to the Excel baseline, for every device. A device that already has the local flag but finds no cloud marker **backfills the marker instead of importing**; if the marker can't be read at all the importer does nothing rather than guess. Note the local flags are **per-origin**, so `localhost` and the deployed site track them independently — but they share the one Firestore, so the marker governs both.
...
| `cc_acc_excel_v7` | `accounting.js` | Excel-import fast path only; the real gate is `acc_settings/main.excelImport` (see importers) |
