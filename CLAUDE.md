...
- **Enforcement is OFF and must stay off until the console's verified % is ~100.** Turning it on early rejects live order writes — and because every service catches and falls back to `localStorage`, the customer still gets the WhatsApp receipt and confirmation email while the order silently never saves. Always place a real test order immediately after enforcing.
