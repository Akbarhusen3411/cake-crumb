...
npm run preview           # Serve the built dist/ — the ONLY way to test the CSP (see App Check & the CSP)
...
npm run deploy            # Build + copy index.html → 404.html + stamp sitemap lastmod + push dist/ to gh-pages
```
...
**A backtick inside a component's `<style>{` … `}</style>` block ends the string.** Several components (`ProductQuickView` especially) ship a real stylesheet inside a template literal. A backtick anywhere in there — including inside a CSS `/* comment */` quoting a property name — closes it early and fails as `Parsing error: Unexpected token <whatever followed>`. Hit three times in one sitting. After editing one of those blocks:

```bash
# no stray backticks, and braces still balance
node -e "const s=require('fs').readFileSync('src/components/ProductQuickView.jsx','utf8');const a=s.indexOf('<style>{\`')+9,b=s.indexOf('\`}</style>');const c=s.slice(a,b);console.log('backticks',(c.match(/\`/g)||[]).length);let d=0;for(const ch of c.replace(/\/\*[\s\S]*?\*\//g,'')){if(ch==='{')d++;else if(ch==='}')d--}console.log('depth',d)"
```
...
- **App Check is attached inside `getFirebaseApp()`, and the order is load-bearing** — after `initializeApp()` and before any Firestore or Auth call, or the first request goes out unattested. It is gated on `VITE_RECAPTCHA_SITE_KEY` and **returns early when that is unset**, which is what keeps `npm run dev` working for anyone without the key. It never throws, and that rule survived enforcement being switched on: the failure it guards against is a reCAPTCHA hiccup, and throwing here would take the storefront's Firestore access down with it rather than let one request go unattested. **`/privacy` states that reCAPTCHA runs on every page**, so unsetting that var makes a published claim false — see *Policy pages*.
...
- Firestore rules live in `FIREBASE_SETUP.md`, not in code. `orders` are admin-only reads; the public `tracking` mirror and `reviews` are world-readable; all validated-create. **`reviews` create uses `hasOnly([...])` + size caps**, so adding a review field means updating the console rule or creates silently fail. **A field named in a validation line but missing from `hasOnly` is unsatisfiable** — that shipped: two leftover `email is string` lines outlived the removal of `email` from the list, so a create carrying an email failed `hasOnly` and one without it failed the type check, and **no review could be written for months**. When editing that rule, check the two lists agree. Changing a collection's shape **or adding one** needs a rules update or production breaks.
...
### App Check & the CSP — the two server-side-ish controls

Neither is optional decoration: `orders`, `tracking` and `reviews` all accept **unauthenticated creates** (they must — customers never sign in), so the Firestore rules validate *what* is written but not *who* writes it. Anyone can lift the Firebase config out of the published bundle. **Nothing in a client-side bundle can be protected by obfuscation — don't propose it.**

- **App Check** (`initAppCheck()` in `firebase.js`) attests that a request came from the real site. It runs **inside `getFirebaseApp()`, before `getDb()`/`getFirebaseAuth()` can hand out a service** — a request that leaves first carries no token. It **never throws**; enforcement is a console decision, not a code one. No `VITE_RECAPTCHA_SITE_KEY` ⇒ no-op, matching how Firebase and EmailJS already degrade.
- **reCAPTCHA Enterprise, not the v3 provider** (`ReCaptchaEnterpriseProvider`). The console marks plain reCAPTCHA deprecated for new integrations; Enterprise is free to 10k assessments/month and works on the no-cost **Spark** plan. It issues **one key ID and no secret**. The key is **score-based** (`Challenge: No`, locked at creation) — so there is never a checkbox, and a low-scored visitor has **no way to appeal**.
- **Enforcement for Cloud Firestore is ON (16 Sep 2026).** An unattested write is now rejected, not merely counted. It was held on Monitoring until the console read ~100% verified, because turning it on early rejects live order writes — and since every service catches and falls back to `localStorage`, the customer still gets the WhatsApp receipt and confirmation email while the order silently never saves. **That failure mode is now always one console toggle away**: anything that breaks attestation (an expired key, a new origin, a provider swap) looks exactly like a healthy site from the customer's side. `Unenforce` in the console reverses it within a minute and needs no deploy.
- **Verifying App Check — the console's metrics, not DevTools.** App Check → APIs → Cloud Firestore splits requests into verified / outdated client / unknown origin / invalid. Read a **recent** window: the 7-day figure averages in the days before it shipped and reads low for weeks afterwards. `unknown origin` means no token at all; **`invalid` means a token that failed validation**, which in practice is a debug token the console doesn't hold.
  - **A missing `exchangeDebugToken` request is NOT a failure.** The token is cached in IndexedDB for its TTL (1 day), so the exchange fires once per browser profile and never again — and an incognito window keeps that cache until *every* incognito window closes. Chasing it in DevTools costs an hour and proves nothing; the metrics answer it in a minute.
  - What a healthy localhost looks like: `recaptcha__en.js` and `anchor?ar=…&k=<site key>` load from `www.google.com` (so the Enterprise key does accept `localhost`), the console prints `App Check debug token: …` — **that line is printed unconditionally in debug mode and is not an error** — and no `[firebase] App Check init failed:` appears.
  - **Enforcement takes up to 15 minutes to start.** A test order placed straight after pressing Enforce proves nothing.
  - **Test by placing a real order and checking `/admin/orders`.** Never read the WhatsApp message or the confirmation email as proof — both fire independently of `saveOrder()` and arrive looking normal over a rejected write. `/track-order` is worth its own check: the public `tracking` mirror is a second collection with its own create. Then open `/admin/accounting` on localhost — if the figures load but a new entry doesn't survive a refresh, that's the silent `localStorage` fallback and the debug token is the cause, not the code.
- **Dev uses a debug token from `.env`** (`VITE_APPCHECK_DEBUG_TOKEN`), not `FIREBASE_APPCHECK_DEBUG_TOKEN = true`. With `true` the SDK invents a token and prints it to paste into the console, which masks it instantly — one wrong character is then unreadable on both sides and shows only as a permanent 403 that looks exactly like a broken project. Generate it in the console instead. Cost an afternoon once.

**CSP** lives in `vite.config.js` as `cspPlugin`, injected as a `<meta>` at **`head-prepend`** (a meta CSP only governs what follows it). GitHub Pages cannot set headers, hence meta — with two inherent limits: **`frame-ancestors` is ignored** (no clickjacking protection available), and **there is no Report-Only mode**, so it cannot be staged.

- **`apply: 'build'` deliberately** — the dev server injects the React Refresh preamble as an inline script that `script-src 'self'` would block. Test with `npm run preview`, **never** `npm run dev`.
- **`script-src` carries no `'unsafe-inline'` and must not.** That keyword is the whole point. The only inline blocks are `type="application/ld+json"`, which browsers never execute. `style-src` does need it (components ship real `<style>` blocks).
- **Adding any integration means adding its origin here**, or it fails in production only. Every origin present was read off the SDKs and the built HTML, not recalled.
...
**Both order paths must READ what `saveOrder()` returns.** It resolves to `{ …, firebaseId }` — `null`, plus an `error`, when the cloud write failed — and `Checkout.jsx` and `ChatBot.jsx` discarded it for months. That was survivable while App Check sat on Monitoring, because an unattested write still landed. **Under enforcement it is rejected**, and side-effects 1, 3 and 4 all fire regardless, so the customer gets the WhatsApp receipt and the confirmation email over an order nobody received — the one failure invisible from the customer's side. Both now branch on it **without awaiting it** (the success screen must not wait on a network round trip): the WhatsApp band turns amber and becomes an instruction rather than reassurance, and **Track Order is hidden**, since it reads the `tracking` doc from that same save and would answer "order not found". `isFirebaseEnabled` separates a failed write from a dev build with no cloud configured. The **`tracking` mirror reports itself too** (`tracked`): it is a second write to a second collection and can fail on its own — a rule change, or an App Check rejection the `orders` write survived. It used to be caught, logged and reported as success, so Track Order offered itself and answered "order not found", which reads as a lost order rather than a link that was never going to work. A false `tracked` hides the link **and nothing else**: the order did save, so the band stays green.
...
- **Nothing is delivered on a self-pickup order, so nothing may say "Cash on Delivery".** The stored `payment.method` stays `'cod'` — the bulk rule, the totals, the Firestore rules and the invoice all read it — but every **label** follows `deliveryMethod`: the checkout's payment tab and recap, the success screen, the WhatsApp receipt, `paymentLabel()` in `AdminOrders.jsx` and `payment_method` in `emailNotify.js`. The sub-labels said "pay in cash when you collect" for months while every heading above them promised a van. A deposit order's balance line follows the same rule ("₹X on pickup").
...
- **Both entry sheets are one layout** — `OrderForm.jsx`, and `ExpenseForm` inside `ExpensesTab.jsx`. A `.cc-sheet` workbench: the item list takes the width and the height (`.cc-sheet__main`), and everything asked once per order — date, customer, payment, status — sits in a column beside it (`.cc-sheet__side`). The details are written **first in the DOM** and sent right with `order: 2`, so a phone asks for them first while a laptop leads with the list; the panes stack below 992px. A field-per-line card meant two items filled the screen, and a thirty-item bill now scrolls one list while the total, the details and Save all stay put.
  - **One scroller per pane, never one inside another.** The old sheet scrolled the dialog body **and** the item grid inside it, both with `overscroll-behavior: contain` — on a shorter laptop screen the wheel stopped dead at the end of the inner list and the sheet read as frozen. That was the "it sticks on some laptops" report. `Modal`'s **`full`** + **`flush`** exist for this; the flush body is a **flex parent** above 992px, so the sheet inherits its height by stretching rather than resolving a percentage against a flex item — the percentage path is the one that quietly gives up and lets the panes collapse.
  - One item is **one row** of `.cc-stable` (`# · Category · Item · Size · Qty · Rate · Total · ⌫`; expenses `# · Item · Qty · ₹ each · Total · ⌫`). Column widths are declared **once on the frame** as `--cc-cols` and inherited by the header row and every item row, so the two can't drift. The minimums are sized to the narrowest two-pane window — a 1024px laptop leaves the list ~623px and it needs ~600px — and **`min-width: 0` on the inputs** is what lets them shrink into their track, since an `<input>` carries an intrinsic width from its `size` attribute and would otherwise push the last column out of the frame.
  ...
  - The **total band closes the table** (`.cc-stotal`) and the same figure rides in the footer beside the buttons (`.cc-sheet__foottotal`, so it's still in front of you once the details column has scrolled away on a phone) and in the details column's summary card (`.cc-ssum`). A separate bill recap under the table was tried and removed — the Total column already says it per item.
  - **Exactly one add control.** `.cc-saddrow`, full width, **outside** the scroller so it stays under the last visible row however long the list gets. A labelled pill in the pane header was the same button twice on one screen and went, taking `.cc-additem` / `.cc-oadd` with it. **Enter on Qty (orders) or ₹ each (expenses)** opens the next line, which is what makes a long bill typeable without the mouse. `.cc-stable` is **`flex: 0 1 auto`** — it hugs its rows, so three items don't leave a white void above the total band, and shrinks to scroll once the list outgrows the pane.
  - **Written for a long list, not a short one.** `buildMenuIndex()` indexes `acc_menu` once into category → item names → sorted sizes; rows are `memo`ised and every callback handed to them is stable (`addLine` reads a `linesRef` synced in an effect, **not** `lines`, or a fresh `onAddRow` on each keystroke would re-render every row and buy nothing). Each row used to map + Set + sort all 207 menu rows three times over — sixty times over for a twenty-item order, which is what made it crawl.
  - **Paid / Cash / Status are pressed, not picked** (`.cc-seg`). Three identical grey selects were three taps each on a touchscreen, and nothing on the sheet said which answer was the usual one. Tones: `--ok` solid green for Paid, `--warn` an amber **tint** for unpaid (matching `.cc-row-unpaid`; amber dark enough to carry white text stops reading as the same colour), `--danger` for Cancelled.
  - **Every colour here was measured against its own background**, not eyeballed: four of them shipped between 2.1:1 and 4.3:1 where small text wants 4.5:1. Check a new one before using it, and remember the background is `#fffaf8` or `#fdeef2` in these panes, not white.
  - Mistakes are flagged **on the row**, not only in the alert at save: `.cc-sline--noprice` (a named item with no rate — the one mistake that books a ₹0 order) and `.cc-sline--short` (a per-piece item still on 1). The save alerts themselves are unchanged.
  - Below 768px the row folds from grid to flex, each item becomes its own boxed block, and `.cc-sline__cap` captions appear over Qty / Rate / Total — the table header is hidden there, so nothing else names them. The inner scroll is dropped; the sheet's own scroll does it.
  - **Expense-only** — vendor and item names are offered from what's been typed before through **two `<datalist>`s** (one element each for the whole list, whatever the row count; deduped case-insensitively like the customer list). Delivery is a details-column field, and the summary states items / delivery / total separately: delivery is part of the total but isn't one of the things bought.
- **`Modal.jsx`** — a flex-column sheet inside a **non-scrolling backdrop**: header and footer pinned, `.cc-modal-body` scrolls (`overscroll-behavior: contain`). Widths `480 / wide 640 / xl 900`, plus **`full`** at 1180 — which also takes `height: 100%`, so its item list has a stable place to scroll instead of growing the dialog off-screen, and **`flush`** drops the body's padding and its scrolling (see the entry sheets). `icon` draws a badge beside the title. It is a **labelled `role="dialog"`** (`aria-modal`, `aria-labelledby` on the title) that takes focus on open and hands it back to whatever opened it on close. **Deliberately NOT a focus trap** — `SearchableSelect` portals its list onto `<body>`, outside the sheet's subtree, so a trap would refuse focus to the one control the order sheet is mostly made of. Edge-to-edge full height on a phone. **Never `align-items: stretch` on the backdrop** — it sizes the sheet to the viewport and taller content spills outside its own white background, so the page behind shows through between the fields.
...
- **My Money is paged** like every other list (it used to render every row ever entered) and carries the shared `PeriodSelect`; its In/Out totals follow the *filter*, not the page.
- **Orders tab extras** — a **Today strip** (`.cc-today`: count · received · to collect, before any filter, cancelled excluded) with a Today ⇄ All-dates toggle, since every other figure on the page is a month or all-time; **Status filter chips** matched on the lower-cased status; a list total that **excludes cancelled** (as `computeSummary` does) and says how many it dropped; and **customer names that filter to themselves** (`.cc-name-link`). An online order carries a **`.cc-bankchip`** (In bank ⇄ ✓ Taken to cash) which is **`white-space: nowrap`** — wrapped to two lines it made that one row half again as tall as its neighbours; the rest of the explanation is on its tooltip and in a single `.cc-acc-note` line above the table, not the three-line paragraph that used to stand there. The five row actions sit in one **`.cc-row-actions`** group, evenly spaced with a hairline setting Delete apart from Edit. The Dashboard's "still to collect" opens Orders pre-filtered to unpaid through `AdminAccounting`'s `ordersPreset` — tabs mount fresh, so `OrdersTab` reads it in its **state initialisers**, no effect; any tab pill clears it.
...
- **Admin chrome** — `App.jsx` hides the storefront shell on `/admin/*` and renders `AdminHeader.jsx`; the brand mark is **deliberately not a link** (it would be an accidental exit). `AdminNav.jsx` is the private Orders ⇄ Accounting nav, never linked publicly; it takes an optional **`className`** so a page can drop its standalone row and sit the pills inside its own header line — `/admin/accounting` does, putting title · refresh · switcher · Lock · Sign out on **one** `.cc-acc-head` row. Four stacked bands (nav, title, tabs, today strip) spent a third of a laptop screen before the first figure. The tab pills (`.cc-acc-tabs` / `.cc-acc-tab`) carry a hairline under the row so they read as a tab bar rather than pills floating on nothing. The five list tabs swap to **stacked cards on mobile** via `useIsMobile()`.
- **Admin mobile classes (≤768px)** in `index.css` — reuse rather than reinventing per tab: **`.cc-admin-tabs`** (horizontal snap strip, bleeds to the edge so a half-pill signals more), **`.cc-admin-toolbar`** (stacks button/search/count; the search's inline `maxWidth` means the override needs `!important`), **`.cc-admin-filters`** (labels pinned to a fixed column so chips align), **`.cc-row-action`** / **`.cc-row-actions`** (19px icons with a real hit area, vs Bootstrap's 14px `btn-link`, grouped and evenly spaced), **`.cc-card-actions`/`.cc-card-action`** (card actions as centred icon tiles — labelled buttons plus one bare icon sat unevenly and ate the card's width; the label lives on `title`/`aria-label`).
...
**`ProductQuickView`'s desktop image is `position: absolute; inset: 0`, and must stay that way.** It was `height: 100%`, which on an `<img>` in a grid row of indefinite height resolves to `auto` — so the photo fell back to its own aspect ratio and *the photo decided the modal's height*. A portrait shot left a tall white gap under the price rows; a landscape one left none, so the same product type opened at three different sizes. Out of flow, the info column is the only thing setting the height. `.qv-image-col` needs `overflow: hidden` **and the image's border-radius**, or the settle-zoom pushes rounded corners past the square column and they flash sharp.
...
**A field error must be reachable, not just visible.** Every checkout field carries `aria-invalid`, and each error `<div>` has an `id` + `role="alert"` with the input pointing at it through `aria-describedby` under the same condition — `aria-invalid` alone announces "invalid entry" and never says why. Fields with no visible `<label>` carry an `aria-label`: a placeholder is gone the moment you type. Keep both when adding a field.
...
**`<lastmod>` is stamped from git at build time**, by `scripts/stamp-sitemap.js` as the last step of `predeploy`. Each URL takes the committer date of the newest file that decides what it renders (its component plus the data it reads), so a price edit in `products.js` moves `/shop` and `/menu` and nothing else. Stamping today's date on every URL each deploy would be **worse than the nothing that was there before** — Google ignores `lastmod` on sites where it isn't consistently accurate. **`/reviews` is deliberately left undated**: its content is Firestore reviews that change without a commit, so any git date would be a confident lie. It runs on `dist/`, never `public/`, so a deploy leaves nothing modified in the tree, and it can never fail a build — no git, no sitemap or an unmapped URL each leave that entry as it was.
...
- **No "Verified Buyer" badge.** It rendered unconditionally on every review while nothing could verify a purchase — the form is public and its "Your Order" field is a free choice from the product list, not a link to an order. A test review placed with no order at all was badged Verified Buyer, which is how it was found. Fake verification badges are what consumer-protection rules on reviews target and what Google penalises by hand. There is no honest version while orders and reviews are unlinked, so it is gone rather than gated.
...
- **`/privacy` must disclose reCAPTCHA.** App Check runs it on every page, sending Google the visitor's IP, browser/device details and interaction signals, and setting a `_GRECAPTCHA` cookie — so "cookieless analytics" is no longer a statement about the whole site, only about Plausible. Google Fonts is named too, as the one other third party that sees a request. If App Check is ever removed, remove that section with it.
...
- **Brand kit** — Playfair Display (headings/wordmark), Lato (body), Allura (accents); CSS vars `--font-heading`/`--font-body`/`--font-script`. Palette on `:root`: `--cc-rose #e0617a`, `--cc-rose-deep #cf3e63`, `--cc-rose-soft #d7a7ae`, `--cc-blush-soft #f3d7d9`, `--cc-cream #fff6f2`, `--cc-blush #f7e3df`, `--cc-cocoa #5b3e36`, `--cc-cocoa-soft #7a584d`. **`--cc-rose-ink #ad2f4e` is rose for SMALL TEXT and only that** — `--cc-rose` is 3.2:1 on cream and `--cc-rose-deep` 4.36, both under the 4.5 an 11px word needs, which was 19 of Lighthouse's contrast failures in one stroke. Use `--cc-rose` for fills, borders and icons, where contrast doesn't apply; switch the moment the rose is a word someone has to read. It clears 4.5 on the **blush** background too (`#f3d7d9`), not just cream — an earlier `#c23a5c` passed on cream and still failed there.
- **Button motion is one system, not per-component.** Three tokens on `:root` — `--cc-ease` (`cubic-bezier(0.22, 1, 0.36, 1)`), `--cc-dur` (180ms), `--cc-dur-fast` (110ms) — drive `.btn-rose`, `.btn-outline-rose`, `.cc-product-card__add`, `.cc-place-order-btn`, `.cc-clear-cart`, `.qty-btn` and the quick view's `.qv-btn`. Every one has hover lift + shadow, a **faster `:active` press** (the thing that actually makes the site feel responsive — only Place Order had one before), and a focus ring. Retune from the tokens; don't add a fourth duration. Transitions name their properties — **never `transition: all`**, which also animates whatever a later edit adds. One `prefers-reduced-motion` block disarms every transform and the primary CTA's sheen; keep new buttons on that list.
...
- **The brand yields, the nav never does.** `.cc-brand` carries `min-width: 0` + `overflow: hidden` and `.cc-header__right` carries `flex-shrink: 0`. Without that pair the wordmark — which is `white-space: nowrap` inside a shrinkable flex item — doesn't wrap or clip when it runs out of room: it **overflows and paints on top of the nav links**, which is exactly what a larger mark caused. The wordmark and tagline are `clamp()`d rather than stepped so they give way smoothly. **The tagline is the widest thing in the lockup** — 41 characters of script, wider than the wordmark above it — so it, not the mark, decides how much room the brand needs on a phone; it hides below 576px.
- **Sticky header** — `.cc-header` with a scroll-aware shadow; other sticky elements use `top: calc(var(--cc-header-h, 81px) + 1rem)`. **Don't hard-code pixel offsets.** **Critical:** `html, body` use `overflow-x: clip` — **never `overflow-x: hidden`**, which turns body into a scroll container and kills `position: sticky` on the header.
...
- **Image pipeline** — drop rasters into `/public`, run `npm run optimize-images` (`scripts/convert-to-webp.js`, quality 78, skips fresh files). Originals are kept but the runtime always requests `.webp`.
  - **Pair `srcSet()` with a `sizes`, or a 700px master lands in a 120px slot.** That was 789 KiB of waste on the home page alone: the wordmark (164 KB, drawn at ~120px) in the header, the mobile footer, the ChatBot header and its message avatar, plus four featured cards pulling full product photos into half-width phone slots. Down to 173 KiB once every one named the `-400`/`-800` variants.
  - **`public/logo_final.png` is TRIMMED to the drawing** (892×621). It arrived on a 1536×1024 canvas with 339px of empty space below the artwork, so every box it was given rendered 39% nothing — the mark looked small and sat visibly high, and a 96px header slot showed a 58px picture. Trim the **master**, not the outputs: `convert-to-webp.js` does a straight resize and would undo a patched variant on the next run. Re-trim if the artwork is ever replaced.
  - **`width`/`height` must describe the file actually SERVED, not the master.** Variants are resized from the master, so their dimensions are the ones to declare — an untrimmed master over a trimmed variant is a real aspect-ratio mismatch and costs a Best Practices point. Declaring the master's numbers over a variant is a real aspect-ratio mismatch and costs a Best Practices point. Measure the variant.
...
- **Anything added above `.cc-header` must sit OUTSIDE it.** That element is `position: sticky` and `--cc-header-h` is hardcoded — **81px / 73px** — and the Shop sticky cart, `.cc-product-card` scroll-margin, the checkout anchor and `#our-story` all measure from it. Growing the header without moving the variable misplaces every one of them, silently.
  - **The variable = `.logo-icon` + the inner `.container`'s `py-2` (16px) + the 1px border.** Those three are one number: change the mark's height or the padding and the variable has to follow. It read **82px while the header was really 111px** for a long time, so everything measuring from it sat 29px too high, tucked under the header — found by screenshotting the live site and scanning a pixel column for the bottom edge, which is how to check it rather than trusting the arithmetic.
  - **Below 380px the formula stops applying**: the mark is shorter than the icon group beside it, so the icons set the height and it stays at the 575 breakpoint's 73px. There is deliberately no override there.
...
- **A React-rendered LCP image is invisible to the preload scanner.** The home hero carries `fetchPriority` and isn't lazy, and Lighthouse still failed it on "discoverable in the initial document" — the browser only learns the image exists once the bundle parses. `index.html` preloads it with **`imagesrcset` + `imagesizes` mirroring the `<img>` exactly**; mismatched, the preload fetches a *second* copy instead of saving a trip. Live LCP went from 7.2s to a median 3.3s. Any future above-the-fold hero needs the same pair.
- **An `<img>` with `height: 100%` needs a parent with a DEFINITE height.** Without one it resolves to `auto`, the photo falls back to its own aspect ratio, and **the photo decides the layout's height**. It has bitten twice: `ProductQuickView`'s desktop image opened the same modal at three different sizes depending on the shot, and Home's About band ran ~1280px tall on a wide screen because `cake-biscoff-drip-top` is 1200×1600 portrait at 50vw. Three ways to give the parent a height, all in use here — an **`aspect-ratio`** on the wrapper (every hero, `.ig-tile`, `SmartImage`'s own span), a **fixed pixel height**, or taking the image **out of flow** (`position: absolute; inset: 0`) inside a column that has one. Swept the whole stylesheet on 16 Sep 2026: 38 image classes, all covered. A new one must pick one of the three.
...
| `VITE_APPCHECK_DEBUG_TOKEN` | Dev only. Without it the SDK invents a debug token and prints it — see *App Check & the CSP* for why that path burns an afternoon. Generate it in the Firebase console instead. Never set for a build. |
...
**Lighthouse (live home page, mobile).** Accessibility, Best Practices and SEO are all **100** — keep them there. What that cost, so it isn't undone:
every icon-only control needs an accessible name (the newsletter button's label is `d-none` below `sm`, so on a phone it was an icon and nothing else);
a link's **visible words** must say where it goes, because the `link-text` audit reads them and not an `aria-label` ("Learn More" became "Read Our Story");
an `aria-label` must **contain** the visible text or it fails WCAG 2.5.3 (the logo's said "Cake & Crumb home" over a wordmark that reads its tagline too —
removing it was the fix); headings may not skip levels; touch targets are 24×24 minimum.

**Performance sits around 85 and roughly a third of the gap is unreachable.** `cache-insight` alone is ~1,400 KiB and GitHub Pages cannot set headers —
the same constraint that forced the CSP into a `<meta>`. Of what remains, render-blocking (~840ms) is Google Fonts plus the app CSS, so closing it means
self-hosting fonts or inlining critical CSS, both real trade-offs on a typography-led brand; the 31 KiB of unused JS is inside `react-vendor`. **Lighthouse
here is noisy** — three runs of one build gave 61/85/92. Take a median, and quote the deterministic numbers (bytes, CLS) rather than the score.

**Checking responsive layout — `scrollWidth` is useless here.** `html, body` use `overflow-x: clip`, so content that spills is silently cut instead of
producing a scrollbar: `document.scrollWidth` never exceeds the viewport and a sweep based on it reports a clean site while things are visibly broken. Walk
the elements instead — anything whose `getBoundingClientRect().right` exceeds `clientWidth` is spilling. Drive it over CDP: launch Chrome with
`--remote-debugging-port` + `--headless=new`, read the target from `/json/list`, and talk to it with **Node's built-in `WebSocket`** (no `ws` dependency
needed), using `Emulation.setDeviceMetricsOverride` to step through widths. A sweep of the 10 public pages at 320 → 1920 takes about three minutes.

**Two things always show up in that sweep and are both deliberate:** the ChatBot panel's decorative blur (`pointer-events-none`, `-top-10 -right-10`), and
the gallery's video rail, which is an `overflow-x: auto` scroller with fixed 200px tracks — see *Video*. Anything else is a real break.
...
**`og:image` is a purpose-made card, not a product photo** (`public/og-card.jpg`, 1200×630). Every product shot is portrait, and the tag used to name one while *declaring* 1200×630 — so WhatsApp and Facebook laid out a landscape preview and received a 1500×2000 portrait. Cut from `products/milkcake-assorted-tubs.webp` (1600×900, so it loses 30px top and bottom and nothing at the sides); the `sharp` command is in a comment above the tag. **JPEG on purpose** — those crawlers are reliable with it and patchy with WebP. If the photo changes, the declared width/height must change with it. WhatsApp caches previews hard: test with a `?v=2` URL, not the plain one.
