...
- **Data** — `src/services/accounting.js` mirrors the `orders.js` shape (Firestore when configured, always mirrored to `localStorage`, never throws). Four **admin-only** collections: `acc_orders`, `acc_expenses`, `acc_withdrawals`, `acc_menu`. Both orders and expenses are **itemized bills**: `acc_orders` = `{ date, customer, items: [{category, item, variant, qty, unitPrice}], amount, paid, method, status, notes }` (one customer, many items — `orderTotal()`/`orderQty()` in accounting.js sum the lines and fall back to legacy top-level `qty`/`unitPrice` for old single-item docs); `acc_expenses` = `{ date, vendor, method, notes, items: [{name, price}], delivery, amount }` where `amount = Σ item prices + delivery`. All money summaries read `amount` / `orderTotal()`, so line-items are additive detail, not a breaking change. **Adding these means adding Firestore rules** (`allow read, write: if request.auth != null` for each) or cloud writes silently fall back to localStorage — the snippet is in `ACCOUNTING_SETUP.md`. **Be aware of how loud that failure isn't:** every helper catches and falls back to `localStorage`, so a broken Firestore renders identically to a healthy one — a device shows its own stale rows while the cloud stays empty, and two devices silently disagree. Worse, the local mirror makes a failed write *look* saved right up until the next read from the cloud reverts it. Failures now route through `reportCloudError()` → `onCloudError(cb)`, which `AdminAccounting.jsx` renders as a "Not saved to the cloud" banner — keep new cloud calls on that path rather than a bare `console.error`. When cloud and local diverge, still don't trust the rendered figures; read Firestore directly (a `getDocs` with **no** fallback) before diagnosing anything.
- **Live, not one-shot** — the page subscribes via `subscribeDocs(coll, cb)` (`onSnapshot`, mirrors each snapshot to `localStorage`) for all four collections, so an entry made on one PC appears on the other without a refresh. `listDocs()` remains the one-shot read behind the ⟳ button and the importers. With no Firebase, `subscribeDocs` delivers `localStorage` once and never again — which is why the tab components still call `reload()` after a save; don't remove those.
...
- **Money model — working capital, four numbers.** The owner reuses the same money: put ₹15,000 in → buy ₹15,000 of material → sell ₹20,000 (₹5,000 profit, with the original ₹15,000 back inside that ₹20,000) → spend ₹10,000 of it on more material, so expenses total ₹25,000. That model *is* the dashboard:
  - **Money in hand** = invested + received − expenses − taken out (`s.moneyInHand`)
  - **Profit** = received − expenses (`s.profit`) — money the owner puts in is **not** income, and money taken out is **not** a cost, so neither touches profit
  - Cash-basis, no stock tracking: profit dips negative right after a big purchase and recovers as those orders sell. That's expected; don't "fix" it with an inventory concept unless asked.
- **`direction` on `acc_withdrawals`** — `'in'` (owner invests own money) or `'out'` (owner takes money for personal use); **a row with no `direction` counts as `'out'`**, which is what every row meant before investing existed. Deliberately reuses the existing collection rather than adding `acc_investments`, so **no new Firestore rule is needed** — a real consideration here, since a missing rule silently degrades to localStorage. Note the `acc_*` rules are `allow read, write: if request.auth != null` with **no `hasOnly` field list**, so adding a field to any of them needs no rule change (unlike `reviews`, where it would break creates silently). `OwnerMoneyTab.jsx` (the "My Money (In/Out)" tab) is the one writer.
- **Order status is tracking only** — `Completed | In Progress | Pending | Cancelled` (`OrderForm.jsx`), colour-coded in the list via `statusInk()`. **Only `Cancelled` changes any figure** — `computeSummary` drops it from sales; the rest are for the baker's own view of what's still in the oven. Adding another status is safe; renaming `Cancelled` is not.
- **Two pockets** — every order/expense/owner-money row carries `method: 'Cash' | 'Online'`. **Cash in Hand** = cashReceived + investedCash + onlineTakenToCash − cashExpenses − cashWithdrawn; **In Bank (Online)** = the online equivalents minus what's been moved to cash; **Money in Hand** = their sum. The Dashboard shows these as one small line (all-time only — for a single month they're that month's movement, not a balance). The `withdrawn` "✓ Taken to cash" toggle on an online order is what feeds `onlineTakenToCash`.
- **One profit definition** — `s.profit`: money actually **received**, not invoiced. An unpaid order sits in `toCollect` until it's paid. Both `DashboardTab` and `MonthlyTab` read this one value; they used to compute their own and quoted different profits for the same month (Dashboard netted `received`, Monthly netted `totalSales`). The Dashboard chart's series is labelled **Received**, not "Sales". `s.totalSales` is still computed but nothing renders it. Keep any new profit-ish figure reading `s.profit`.
- **Searchable dropdowns** — `src/components/admin/SearchableSelect.jsx` (type-to-filter, keyboard nav, `allowCustom` for free text). The New Order form (`components/admin/accounting/OrderForm.jsx`) is a Category → Item → Size/Variant cascade that auto-fills price. Tab components live in `components/admin/accounting/`. The customer list is deduped **case-insensitively** — `allowCustom` means "makbul varisali" and "Makbul Varisali" would otherwise be two customers with split histories.
- **Shared date helpers** — `src/utils/adminDate.js` (`todayIso`, `fmtDate`, `monthLabel`, `inPeriod`); four tabs had their own near-identical copies. **`todayIso()` shifts by the local offset before `toISOString()`** — the plain UTC form dated an entry made before 05:30 IST to *yesterday*. `PeriodSelect.jsx` (All dates / Today / a month) scopes the Orders and Expenses lists; `inPeriod(date, period)` is the matcher. Item labels carry their category (`fullItem()` in `utils/orderItems.js`), skipping it when the name already contains it, so a bare "Chocolate" reads "Chocolate Cake Pop".
...
- **Dashboard layout + the one chart** — `DashboardTab.jsx` is a responsive grid (`.cc-acc-*` classes at the end of `index.css`): hero pair (Money in hand · Profit) → four KPI tiles (Invested · Earnings · Expenses · Taken out) → a one-line still-to-collect / cash / bank summary → a **grouped** bar chart of received vs expenses over the last 6 months. Grouped, not stacked, because the two measures are compared per month and their sum means nothing; one shared axis (never a second y-scale); `MonthlyTab` is the table view of the same figures. The two series colours (`#cf3e63` brand rose, `#6b5bb0`) were run through a palette validator against the actual `#ffffff` card surface — re-validate before changing either. **Figures use `--font-body` (Lato) with `lining-nums tabular-nums`, never `--font-heading`**: Playfair Display is a display serif whose old-style digits sit at uneven heights, so amounts rendered visibly broken. Headings stay Playfair.
- **"Expense taken for use" is gone.** It was one hand-typed figure in `acc_settings/main` standing in for two things the tool now records properly — material bought (Expenses) and owner money moving in/out (My Money) — so it double-counted. `getExpenseTakenForUse()`/`setExpenseTakenForUse()`/`TAKEN_DEFAULT` were deleted with it. `acc_settings` still needs its Firestore rule (nothing reads the doc now, but the rule costs nothing and its absence is a confusing permission-denied). Don't reintroduce a manual money field.
- **Dashboard is deliberately sparse.** It carried Total sales, a received-by-method bar, two big pocket cards and the hand-typed note; the owner called it confusing and asked for "only invest, earning, expense, money in hand, graph". So: hero pair (Money in hand · Profit) → four tiles (Invested · Earnings · Expenses · Taken out) → one small still-to-collect / cash / bank line → the chart. Adding a figure back means removing one; the constraint was the point.
- **Menu importers (flag-gated)** — besides `seedMenuIfEmpty()`, `accounting.js` exports `ensurePerPieceMenu()` and `ensureCakePopPrices()`; each no-ops after setting a `localStorage` flag (`cc_acc_perpiece_v1`, `cc_acc_cakepop_v2`) and bumping the suffix re-runs it. Both only ever *add* a missing menu row, so a repeat on a new device is harmless — which is exactly why they can stay device-gated.
- **The Excel importer is retired — don't reinstate it.** `importExcelDataIfNeeded()` seeded `acc_orders`/`acc_expenses` from `src/data/excelImport.js` (the owner's Apr–Jul 2026 history, ~4.7k lines generated from `cake_and_crumb_excel.xlsx`). The owner **cleared the books to start fresh from August 2026**, and the importer rewrote its `xl-`-prefixed rows with `set` on any device that hadn't run it — so leaving it wired would have refilled the fresh book with the old year. The function is gone, nothing calls it, and `src/data/excelImport.js` now stays in the repo purely as the archive of those figures (unreferenced, so it no longer emits a chunk). `acc_settings/main.excelImport` is a dead marker. Its history is worth knowing if a similar importer is ever written: it was gated on a **localStorage** flag, so a second PC opening `/admin/accounting` for the first time silently reverted every cloud edit made to an imported row — for every device. Any bulk importer must be gated on a **cloud** marker, not a per-device one.
- **PIN gate (`PinGate.jsx`) — a second lock, not a second wall.** `/admin/accounting` asks for a 4–8 digit PIN *after* the Firebase Auth sign-in; `/admin/orders` does not. Be clear-eyed about what it buys: the real boundary is still Auth + the Firestore rules, and anyone holding the admin password can read every figure from the Firebase console or bypass this check in devtools. What it protects is a **signed-in tab left unattended on a shop counter** — which is a genuine risk, and the only one it covers. Don't describe it as securing the data. **The PIN is never hardcoded** (same rule the reviews moderation UI follows): on first use the screen asks the owner to choose one and stores only `SHA-256(salt:pin)` in `acc_settings/main.accPinHash`, so the digits are in neither the published bundle nor the repo. Forgotten PIN → delete that field in the Firebase console and the screen returns to "choose a PIN" mode. Unlock lives in **`sessionStorage.cc_acc_unlocked`**, matching the `browserSessionPersistence` the auth uses, so closing the tab re-locks; `logout()` clears it too, and a `Lock` button re-locks without signing out. A failed *read* of the hash **does not unlock** — it shows an error and a retry, because treating a broken rule as "no PIN set" would turn it into an open door. Data subscriptions are gated on `ready = isAdmin && unlocked`, so nothing loads behind the gate.
- **Invoices (`components/admin/InvoiceModal.jsx`)** — shared by **both** admin pages: a 🗎 button on every accounting order row (desktop table + mobile card) and an **Invoice** button on each expanded website order in `AdminOrders.jsx`. The component renders one **normalised shape**, and `utils/invoice.js` adapts each source into it (`buildAccountingInvoice` / `buildWebsiteInvoice`), so it never branches on which page opened it. A website invoice additionally carries phone, address, fulfilment (pickup/delivery + date), the customer-facing **order ID** as a second reference line, a subtotal/delivery breakdown, and for `deposit` orders a "advance received · balance due" line. **`deliveryKm` is deliberately absent** — the distance is bakery-side only and is kept off every customer-facing surface; an invoice is handed to the customer. Deliberately **print-to-PDF, not a PDF library**: jsPDF/html2canvas would add a few hundred KB and rasterise the page, losing selectable text and the real Playfair/Lato; the browser's print dialog offers "Save as PDF" beside printing on paper. The modal is `createPortal`'d onto `document.body` so `@media print` can `display: none` on `#root` — hiding the app *in place* leaves its layout behind and Chrome prints blank pages after the invoice. Colour lives in **text and borders, never filled blocks**, because Chrome only prints backgrounds when "Background graphics" is ticked. `utils/invoice.js` derives the number `CC-INV-DDMMYY-XXXX` by hashing the doc id — **not a counter**, for the same reason `orderId.js` isn't (a per-day localStorage counter gave two customers the same ID). It needs no Firestore write and is identical on every device. **No GST line** — the bakery has no GSTIN and a tax line without one would be false; the FSSAI + Udyam *numbers* do appear, since displaying FSSAI is required of a food business. `utils/orderItems.js` (`orderLines`/`fullItem`/`oneLine`) is shared with `OrdersTab` so a row and its invoice can't describe an order differently.
- **Danger zone (`DangerZone.jsx`)** — collapsed section at the bottom of `/admin/accounting`. "Download backup" (`exportAccountingBackup()` → a JSON file of all four collections) and "start a fresh book" (`clearAccountingBooks()` → wipes `acc_orders`/`acc_expenses`/`acc_withdrawals` in 400-doc batches, resets the taken-for-use note, **keeps `acc_menu`**), gated on typing `DELETE`. Added when the owner reset the books for August 2026. There is no undo and no server-side backup — keep the confirmation gate and the backup button together.
...
| `cc_acc_excel_v7` | — | Dead: the Excel importer is retired. Safe to ignore; nothing reads it |
| `cc_acc_taken` | — | Dead: the "expense taken for use" setting was removed. Nothing reads it |
