...
- **Money model (two pockets)** — every order/expense/withdrawal carries `method: 'Cash' | 'Online'`. `computeSummary()` (pure, client-side) yields **Cash in Hand** = cashReceived + onlineTakenToCash − cashExpenses − cashWithdrawn, **In Bank (Online)** = the online equivalents minus what's been moved to cash, and **Money in Hand** = their sum. Online payments still count as earnings/profit; "In Bank" is online money not yet withdrawn. The Dashboard's *Money at hand* **is** `s.moneyInHand`, and the two pockets render as their own cards — but **only on the all-time view**, since for a single month those figures are that month's movement, not a running balance.
- **One profit definition** — `s.profit = s.received − s.totalExpenses`: money actually **received**, not invoiced. An unpaid order sits in `toCollect` until it's paid. Both `DashboardTab` ("Profit") and `MonthlyTab` ("Net") read this one value; they used to compute their own and quoted different profits for the same month (Dashboard netted `received`, Monthly netted `totalSales`). The Dashboard chart's series is likewise labelled **Received**, not "Sales" — `MonthlyTab` is the one place both appear, in separate columns. Keep any new profit-ish figure reading `s.profit`.
...
- **Shared date helpers** — `src/utils/adminDate.js` (`todayIso`, `fmtDate`, `monthLabel`, `inPeriod`); four tabs had their own near-identical copies. **`todayIso()` shifts by the local offset before `toISOString()`** — the plain UTC form dated an entry made before 05:30 IST to *yesterday*. `PeriodSelect.jsx` (All dates / Today / a month) scopes the Orders and Expenses lists; `inPeriod(date, period)` is the matcher. Order/expense item labels also carry their category (`fullItem()` in `OrdersTab`), skipping it when the name already contains it, so a bare "Chocolate" reads "Chocolate Cake Pop".
...
- **Dashboard layout + the one chart** — `DashboardTab.jsx` is a responsive grid (`.cc-acc-*` classes at the end of `index.css`): hero pair (Money at hand · Profit) → the two pockets (Cash in hand · In bank, all-time only) → four KPI tiles → received-by-method split → a **grouped** bar chart of received vs expenses over the last 6 months. Grouped, not stacked, because the two measures are compared per month and their sum means nothing; one shared axis (never a second y-scale); `MonthlyTab` is the table view of the same figures. The two series colours (`#cf3e63` brand rose, `#6b5bb0`) were run through a palette validator against the actual `#ffffff` card surface — re-validate before changing either. **Figures use `--font-body` (Lato) with `lining-nums tabular-nums`, never `--font-heading`**: Playfair Display is a display serif whose old-style digits sit at uneven heights, so amounts rendered visibly broken. Headings stay Playfair.
- **"Expense taken for use" is display-only** — the owner's hand-carried figure from the Excel days, reading/writing `acc_settings/main` via `getExpenseTakenForUse()` / `setExpenseTakenForUse()` (`TAKEN_DEFAULT = 12300` until explicitly set; `acc_settings` needs its own Firestore rule like the other four collections). It is **no longer subtracted from anything**. It predates the Expenses tab and meant "money pulled out to buy materials" — with materials now itemised in `acc_expenses` and money leaving the tin recorded in `acc_withdrawals`, netting it off as well counted the same rupees twice. Don't wire it back into a total.
- **Money Taken Out tab** — `WithdrawalsTab.jsx` writes `acc_withdrawals` (`{ date, amount, method, notes }`): money the owner takes for personal/home use. It lowers Money in Hand but **not** Profit, since it isn't a business cost. The component was written, complete, and simply never imported for a while — `acc_withdrawals` was loaded and fed into every summary while nothing could create a row, which is why the manual "taken for use" figure existed at all.
