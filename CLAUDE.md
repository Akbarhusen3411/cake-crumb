...
- **Admin chrome + mobile** — `App.jsx` hides the storefront `Navbar`/`Footer`/`ChatBot`/`BackToTop`/`FestivalBanner` on any `/admin/*` route (`isAdmin = pathname.startsWith('/admin')`), so admin is a clean full-screen tool. `AdminNav.jsx` is the private Orders ⇄ Accounting pill nav on both admin pages (never linked publicly). Orders + Expenses tabs render **stacked cards on mobile** (via `src/hooks/useIsMobile.js`) instead of the wide desktop table.
